...
public static class GalaxyGpt
{
        ...
        ulong[]? allowedChannels = null)
    {
            ...
            message.ReferencedMessage.Author.Id == client.CurrentUser.Id)
        {
            ...
            IMessage? replyrepliedto =
                await message.Channel.GetMessageAsync(((RestUserMessage)replymessage).ReferencedMessage.Id);
...
//                    finalResponseMessage += $"\n\n{ThisAssembly.Git.Commit} | {finalResponse.version}";
        ...
        try
        {
            ...
            if (verbose)
            {
                ...
                if (apiResponse.Duration != null)
                    answerMessage.AppendLine(
                        $"Response Time as seen from GalaxyGPT: {apiResponse.Duration}ms (-API transport overhead)");
                answerMessage.AppendLine(
                    $"Response Time as seen from Ketchupbot-Discord: {idk.ElapsedMilliseconds}ms (+API transport overhead -discord API overhead)");
            }
            ...
            if (!string.IsNullOrWhiteSpace(apiResponse.Context) && verbose)
            {
                ...
                await message.Channel.SendFileAsync(contextStream, "context.txt", answerMessage.ToString(),
            ...
            SocketUserMessage message)
    {
        ...
        while (true)
        {
            ...
            messageToProcess = await message.Channel.GetMessageAsync(messageToProcess.Id) as IUserMessage ??
        ...
        JsonNode jsonResponse = await JsonSerializer.DeserializeAsync<JsonNode>(content) ??
                                throw new InvalidOperationException();
